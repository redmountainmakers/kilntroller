...
function Controller(deviceName, tunings) {
    var self = this;
    ...
    fs.writeFile(
        ...
        function(err) {
            if (err) {
                ...
                self.emit('log', 'History saved to file');
            }
            self.port.close(function(err) {
                if (err) {
                    self.emit('log', 'Error closing serial port: ' + err.message);
                } else {
                    self.emit('log', 'Serial port closed');
                }
                cb(err);
            });
        }
